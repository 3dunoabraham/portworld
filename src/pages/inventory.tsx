    ...
    <div className='flex-center w-100 h-min-100vh'>
        <div className="h-min-100vh w-100 px-8 Q_xs_sm_px-2">
            ...
            <div className='flex'>
                ...
                {isConfigEdit &&
                    <div className='w-100 py-2'>
                        <InventoryForeignsCRUD {...{q__queriedObj:{refetch:()=>{}}, queriedObj:{}}} configObjRef={FETCH_CONFIG} />
                    </div>
                }
            </div>
            {q__unitsArray.isLoading && <LoadingInventory /> }
            {unitsArray.length > 0 && <InventoryPage fetchConfig={FETCH_CONFIG} unitsArray={unitsArray} />}
        </div>
    </div>
    )
}
